const keyonSchemaController = require("../lib/schema");
const iterator = require("../lib/iterator");
const debug = require("debug")("keyon:interface:schemas");

/**
 * Schemas interface management
 *
 * Generic object allowing to regroup all schemas controllers in the same place.
 * It looks like a schemas router.
 */
class keyonInterfaceSchemas {
	constructor(root) {
		this._root = root;
	...
	_node(schema) {
		if(!this.$org[schema]) {
			...
			this.$org[schema] = new keyonSchemaController(this._root, schema)
...
module.exports = keyonInterfaceSchemas;
