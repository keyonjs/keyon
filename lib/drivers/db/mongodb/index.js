...
class keyonDriverMongoDB extends keyonDriverDB {
	...
	modelize(schema) {
		...
		var ret = iterator.assign(schema.fields, (user, dst, object, source) => {
			if(!object.$kind && !object.$kind.type) {
				...
				// get the high type
				const rtype = ko.types[object.$kind.type.toLowerCase()];
				if(!rtype) {
	 ...
	 *
	 * @param {keyonController} schema Schema reference to use
	 * @param {keyonPipeline} pipe Pipeline reference
	...
	update(schema, pipe) {
		pipe.$push(() => {
			schema.model.updateOne(match, {$set: pipe.body})
				.exec((err, doc) => {
				pipe.$next();
			})

		})
		this.get(schema, pipe);
	}
