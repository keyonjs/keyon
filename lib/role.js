...
class keyonRole {
	...
	list(pipe) {
		...
		// clean input
		pipe.$push(() => {
			self._filters.cleanSearchCopy(pipe, pipe.$fifo.bind(pipe));
		})

		// exec early create
		if(access.onCreateEarly) {
			pipe.$push(() => {
				access.onListEarly(pipe)
			})
		}

		// create entry in database
		pipe.$push(() => {
			ctrl.driver.list(ctrl, pipe);
		})
...
*/
	...
	get(pipe) {
		...
		// clean UUID
		pipe.$push(() => {
			self._filters.cleanUUIDCopy(pipe, pipe.$fifo.bind(pipe));
		})
		...
		// clean the output
		pipe.$push(() => {
			self._filters.cleanOutputCopy(pipe, pipe.$fifo.bind(pipe));
		})
	...
	create(pipe) {
		...
		// clean input
		pipe.$push(() => {
			self._filters.cleanInputCopy(pipe, pipe.$fifo.bind(pipe));
		})
